...
public class SimilarityInfo {
	...
	/**
	 * @param w1
	 *            the first word (or it's encoded form)
	 * @param s1
	 *            the first synset
	 * @param w2
	 *            the second word (or it's encoded form)
	 * @param s2
	 *            the second synset
	 * @param sim
	 *            the similarity between the two synsets
	 * @throws JWNLException
	 */
	protected SimilarityInfo(String w1, Synset s1, String w2, Synset s2,
			double sim) throws JWNLException {
		...
		d1 = (iw1 == null ? w1 : iw1.getLemma() + "#" + s1.getPOS().getKey()
				+ "#" + getSenseNumber(iw1, s1));
		d2 = (iw2 == null ? w2 : iw2.getLemma() + "#" + s2.getPOS().getKey()
				+ "#" + getSenseNumber(iw2, s2));
	}
	 ...
	 *            the index word
	 * @param s
	 *            a synset that includes the index word
	 * @return the sense indes of the sysnet for the given word, or -1 if the
	 *         word is not in the synset
	 * @throws JWNLException
	 *             if an error occurs accessing WordNet
	 */
	private static final int getSenseNumber(IndexWord iw, Synset s)
			throws JWNLException {
