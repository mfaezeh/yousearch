...
/*		while (i < data.numAttributes() - 2) {
	...
	protected Instances newCentersAfterSplit(double[] pbic, double[] cbic,
		...
		if ((numToSplit == 0) && (cutoffFactor > 0)) {
			...
			numToSplit = (int) ((double) m_ClusterCenters.numInstances() * m_CutOffFactor);
		}
	...
	protected boolean recomputeCenters(Instances centers, int[][] instOfCent,
		...
		for (int i = 0; i < centers.numInstances(); i++) {
			double val;
			/* YOUSEARCH BEGIN */
/*			for (int j = 0; j < model.numAttributes() - 2; j++) {
				val = meanOrMode(m_Instances, instOfCent[i], j);

				for (int k = 0; k < instOfCent[i].length; k++)

					if (converged
							&& m_ClusterCenters.instance(i).value(j) != val)
						converged = false;
				if (!converged)
			...
			/* YOUSEARCH END */
			  ...
			  for (int j = 0; j < model.numAttributes(); j++) { val =
			  meanOrMode(m_Instances, instOfCent[i], j);
	...
	protected void recomputeCentersFast(Instances centers,
		...
		for (int i = 0; i < centers.numInstances(); i++) {
			...
			/* YOUSEARCH END */
			  ...
			  for (int j = 0; j < model.numAttributes(); j++) { val =
			  meanOrMode(m_Instances, instOfCentIndexes[i], j);
			  centers.instance(i).setValue(j, val); }
	 ...
	 * @return mean value
	 */
	protected double meanOrMode(Instances instances, int[] instList,
			int attIndex) {
		double result, found;
		int[] counts;
	...
	protected boolean assignToCenters(KDTree tree, Instances centers,
			int[][] instOfCent, int[] allInstList, int[] assignments,
		...
		if (tree != null) {
			...
			converged = assignToCenters(centers, instOfCent, allInstList,
					assignments);
		}
		return converged;
	}
	...
	protected boolean assignToCenters(KDTree kdtree, Instances centers,
			int[][] instOfCent, int[] assignments, int iterationCount)
			throws Exception {
		...
		kdtree.centerInstances(centers, assignments, Math.pow(.8,
				iterationCount));
		boolean converged = true;
	...
	protected boolean assignToCenters(Instances centers, int[][] instOfCent,
		...
		for (int i = 0; i < numInst; i++) {
			...
			// System.out.println("inst assigned: "+inst.value(2));
	 ...
	 *            of the region
	 * @param model
	 *            the model for the centers (should be the same as that of
	 *            instances)
	 * @return a pair of new centers
	 */
	...
	protected Instances makeCentersRandomly(Random random0, Instances model,
		...
		for (int i = 0; i < numClusters; i++) {
			int instIndex = Math.abs(random0.nextInt())
	...
	protected double calculateBIC(int[][] instOfCent, Instances centers,
		...
		/* YOUSEARCH BEGIN */
		// int numDimensions = centers.numAttributes() - 2;
		/* YOUSEARCH END */
		...
		for (int i = 0; i < centers.numInstances(); i++) {
			loglike += logLikelihoodEstimate(instOfCent[i].length, centers
					.instance(i), mle[i], centers.numInstances() * 2);
	...
	protected double logLikelihoodEstimate(int numInst, Instance center,
		...
		if (numInst > 1) {
			...
			/* YOUSEARCH BEGIN*/
			/*
			double p2 = -(numInst * (center.numAttributes() - 2)) / 2
					 Math.log(variance);
			*/
			/* YOUSEARCH END*/
			/*
	...
	protected int clusterProcessedInstance(Instance instance) {
		...
		for (int i = 0; i < m_NumClusters; i++) {
			double dist = m_DistanceF.distance(instance, m_ClusterCenters
					.instance(i));
			if (dist < minDist) {
				minDist = dist;
				bestCluster = i;
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option(
				"\tfile to write centers to (ARFF format).", "O", 1,
				"-O <file name>"));
	...
	/**
	 * Sets the maximum number of iterations KMeans that is performed on the
	 * child centers.
	 * 
	 * @param i
	 *            the number of iterations
	...
	public void initDebugVectorsInput() throws Exception {
		m_DebugVectorsInput = new BufferedReader(new FileReader(
				m_DebugVectorsFile));
