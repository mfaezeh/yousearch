...
/**
 * An abstract class that addes information content based methods to the top
 * level similarity measure class but doesn't itself define a similarity
 * measure.
 * 
...
public abstract class ICMeasure extends PathMeasure {
	...
	protected Synset getLCSbyIC(Synset s1, Synset s2) throws JWNLException {
		// TODO Handle the different types of LCS handled by the perl version
		// which are
		// 1) Largest IC value
		// 2) Results in shortest path
		// 3) Greatest depth (i.e. the LCS whose shortest path to root is
		// longest)
		// Although in here we only need the IC based one

		@SuppressWarnings("unchecked")
		List<List<PointerTargetNode>> trees1 = PointerUtils.getInstance()
				.getHypernymTree(s1).toList();

		@SuppressWarnings("unchecked")
		List<List<PointerTargetNode>> trees2 = PointerUtils.getInstance()
				.getHypernymTree(s2).toList();
		...
		if (lcs == null && useSingleRoot()) {
			...
			// TODO: Should probably create one of these for each POS tag and
			// cache them so that we can always return the same one
			lcs = new Synset(s1.getPOS(), 0l, new Word[0], new Pointer[0], "",
